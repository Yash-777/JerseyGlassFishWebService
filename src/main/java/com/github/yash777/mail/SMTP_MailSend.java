...
public class SMTP_MailSend {
	boolean isSigned, isEncrypted, isStoreMessage;
	public SMTP_MailSend (boolean isSigned, boolean isEncrypted, boolean isStoreMessage) {
	...
	protected void setSignerKeyData(String resourceIdentifier, String password) throws MailPreparingException, IOException {
		// security.privatekey
		InputStream cerFileStream = fileObj.getCerFileStream(true, security_privatekey);
	...
	public static void main(String[] args) throws Exception {
		...
		SMTP_MailSend obj = new SMTP_MailSend(false, false, true);
		obj.mailSend();
	}
	...
	public void mailSend() throws Exception {
		String configFile = "EmailConf.xml";
...
configSMTP.setDoEncrypt(isEncrypted);
		...
		if (fileName != null) {
			...
			if (compression) {
				...
				fileName += ".gz";
				att.setName(fileName);
				att.setContentType("application/x-gzip");
	...
	public static void compressCommons(InputStream streamSrc, String fileName, OutputStream gipStream) throws IOException {
		...
		//byte[] buf = new byte[10240];  HardCoded Buffer Size
		byte[] buf = IOUtils.toByteArray(streamSrc);
		while (true) {
